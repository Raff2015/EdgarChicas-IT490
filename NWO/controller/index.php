...
if($_SESSION['id'] == NULL) { //Creates a new session
	
	$_SESSION['username'] = auth ($email, $pass);

    if ($_SESSION['username'] == NULL) { //Authorizes users signin
	   	redirect ("Missing or Incorrect Email or Password", "../view/signin.php", '3') ; 
		}
	
    else {
        session_regenerate_id(); //JW - Generates a brand new ID and kills the old one for every login
		$_SESSION['email'] = $email;
        $_SESSION['id'] = session_id();
		ses_login ($_SESSION['id'],$_SESSION['email']);//JW - Adds login information to DB records
		}
